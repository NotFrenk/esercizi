...
class Persona:
    def __init__(self, first_name, last_name):
        ...
        if isinstance(last_name, str) and isinstance(last_name, str):
            self.__age = 0
        else:
            self.__age = None
    ...
    def greet(self):
        if self.__first_name is not None and self.__last_name is not None and self.__age is not None:
            print(f"Ciao, sono {self.__first_name} {self.__last_name}! Ho {self.__age} anni!")
        else:
            print("Dati della persona errati o incompleti")
